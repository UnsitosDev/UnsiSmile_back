...
public class ToothFaceConditionService {
    ...
    @Transactional(readOnly = true)
    public List<ToothFaceConditionResponse> getAllToothConditions() {
        try {
            List<ToothFaceConditionModel> allToothConditions = toothFaceConditionRepository.findAll();
            return allToothConditions.stream()
                    .map(toothFaceConditionMapper::toDto)
